...
describe('movies fetching flow', () => {
  it('Fetches the movies with error', () => {
    const generator = getMovieWithId({ type: 'app/VideoPlayer/LOAD_MOVIE', id: '1416' });
    generator.next();
    generator.next();
    expect(generator.next().value.payload.action.type).toEqual('app/VideoPlayer/LOAD_MOVIE_ERROR');
  });
